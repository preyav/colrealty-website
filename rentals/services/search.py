...
def get_rental_property_types(active_only: bool = True) -> list[str]:
...
def get_rental_cities(active_only: bool = True) -> list[str]:
    qs = Rental.objects.all()
    if active_only:
        qs = qs.filter(status="active")
...
def apply_rental_filters(qs, params: dict):
    ...
    if q:
        ...
        known_cities = {city.lower(): city for city in get_rental_cities(active_only=True)}
    ...
    allowed_property_types = set(get_rental_property_types(active_only=True))
